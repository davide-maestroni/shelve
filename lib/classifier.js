...
async function classify(tabs, existingShelves = {}) {
  ...
  for (const [, { tabs: clusterTabs, vecs }] of clusters) {
    ...
    for (let i = 0; i < clusterTabs.length; i++) {
      if (clusterTabs.length > 3 && sims[i] < threshold) unsorted.push(clusterTabs[i]);
      else kept.push(clusterTabs[i]);
    }
    if (kept.length === 0) { unsorted.push(...clusterTabs); continue; }
  ...
  const unsortedId       = Object.values(existingShelves).find(s => s.isUnsorted)?.id || 'unsorted';
  const unsortedExisting = existingShelves[unsortedId];
  for (const t of unsorted) tabShelfMap.set(t.normalizedUrl, unsortedId);
  newShelves.push({
    id: unsortedId, title: 'Unsorted', customTitle: null,
    color: unsortedExisting?.color ?? '#94a3b8', order: 9999, isUnsorted: true,
  });
  for (const t of tabs) {
    if (!tabShelfMap.has(t.normalizedUrl)) tabShelfMap.set(t.normalizedUrl, unsortedId);
  }
...
async function assignNewTab(newTab, allTabs, existingShelves) {
  ...
  // Build a sparse centroid for each existing non-unsorted shelf
  const nonUnsortedShelves = Object.values(existingShelves).filter(s => !s.isUnsorted);
  const shelfCentroids = new Map(); // shelfId → sparse centroid Map

  for (const shelf of nonUnsortedShelves) {
  ...
  // or if it should just go to Unsorted
  const unsortedId = Object.values(existingShelves).find(s => s.isUnsorted)?.id || 'unsorted';
  if (bestSim < 0.08) {
    // Truly novel — create a new shelf
    return _createNewShelf(newTab, newTabVec, existingShelves, bestSim);
  }
  // Weak similarity — Unsorted, but record the closest shelf as suggestion
  return { shelfId: unsortedId, isNewShelf: false, fitScore: bestSim, suggestedShelfId: bestShelfId, newShelf: null };
}
...
function _createNewShelf(tab, tabVec, existingShelves, fitScore) {
  ...
  return {
    ...
    newShelf: {
      ...
      order: Object.keys(existingShelves).length, isUnsorted: false,
    },
  };
}
...
function _fallbackAssignment(existingShelves) {
  const unsortedId = Object.values(existingShelves).find(s => s.isUnsorted)?.id || 'unsorted';
  return { shelfId: unsortedId, isNewShelf: false, fitScore: 0, suggestedShelfId: null, newShelf: null };
}

function _fallbackSingleCluster(tabs, existingShelves) {
  const id  = Object.values(existingShelves).find(s => s.isUnsorted)?.id || 'unsorted';
  const map = new Map();
  for (const t of tabs) map.set(t.normalizedUrl, id);
  return { shelves: [{
    id, title: 'Unsorted', customTitle: null,
