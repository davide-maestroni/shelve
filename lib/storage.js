...
export async function setTab(normalizedUrl, data) {
  const tabs = await getTabs();
  const prevTab = tabs[normalizedUrl];
  const tab = { ...prevTab, ...data, normalizedUrl };
  // Write local first so the storage.onChanged listener (which fires when _syncTabMeta
  // writes to sync) already sees the tab in local and won't backfill it without isNew.
  tabs[normalizedUrl] = tab;
  await LOCAL.set({ [KEY.TABS]: tabs });
  try {
    await _syncTabMeta(tab);
  } catch (e) {
    // Roll back local write to keep stores consistent (e.g. SYNC_QUOTA_EXCEEDED)
    const current = await getTabs();
    if (prevTab) {
      current[normalizedUrl] = prevTab;
    } else {
      delete current[normalizedUrl];
    }
    await LOCAL.set({ [KEY.TABS]: current }).catch(() => {});
    throw e;
  }
}
...
async function _syncTabMeta(tab) {
  ...
  try {
    ...
    if (e?.message?.includes('QUOTA_BYTES')) {
      const err = new Error('Sync storage is full. Free up space in Settings before archiving more tabs.');
